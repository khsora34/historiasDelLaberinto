...
class Dependencies {
    ...
    private func createDatabaseFetcherProvider() -> DatabaseFetcherProvider {
        return DatabaseFetcherProvider(eventsFetcherManager: EventFetcherManagerImpl(), itemsFetcher: ItemFetcherImpl(), charactersFetcher: CharacterFetcherImpl(), roomsFetcher: RoomFetcherImpl(), movementFetcher: MovementFetcherImpl(), localizedValueFetcher: LocalizedValueFetcherImpl())
    }
}

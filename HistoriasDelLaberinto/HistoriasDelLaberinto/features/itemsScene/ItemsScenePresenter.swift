...
class ItemsScenePresenter: BasePresenter {
    ...
    private func showItems() {
        ...
        for element in protagonist.items.keys {
            ...
            items[element] = item
            let model = ItemViewModel(id: element, name: item.name, description: item.description, itemType: getITemType(from: item), quantity: protagonist.items[element]!, imageUrl: item.imageUrl, tag: i, isSelected: false, delegate: self)
            itemModels[i] = model
            i += 1
        }
    ...
    private func getITemType(from item: Item) -> String {
